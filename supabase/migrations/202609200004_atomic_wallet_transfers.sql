-- Atomic same-currency wallet transfers.
-- Ownership is always derived from auth.uid(); the client supplies no user_id.
begin;
...
begin
  ...
  if found then
    return query
      select v_existing.id,
             v_existing.source_wallet_id,
             v_existing.destination_wallet_id,
             v_existing.source_transaction_id,
             v_existing.destination_transaction_id,
             v_existing.source_amount,
             v_existing.destination_amount,
             v_existing.status,
  ...
  -- Lock both wallets in a stable order so concurrent transfers cannot race
  -- the source-balance check or deadlock when they use opposite directions.
  if p_source_wallet_id < p_destination_wallet_id then
  ...
  select currency, status, deleted_at, name
  ...
  if v_transfer_id is null then
    ...
    return query
      select v_existing.id, v_existing.source_wallet_id, v_existing.destination_wallet_id,
             v_existing.source_transaction_id, v_existing.destination_transaction_id,
             v_existing.source_amount, v_existing.destination_amount, v_existing.status,
             v_existing.reference, v_existing.created_at;
    return;
  end if;
