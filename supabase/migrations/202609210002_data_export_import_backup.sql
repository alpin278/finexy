-- Phase 28: authenticated data export, import, and backup restore.
-- Backup references are opaque and are never used as database identifiers.
...
create table public.backup_imports (
  ...
  constraint backup_imports_key_not_empty check (char_length(trim(backup_key)) = 64),
  constraint backup_imports_version_check check (backup_version = 1),
  constraint backup_imports_mode_check check (mode in ('merge', 'restore_empty')),
  constraint backup_imports_summary_object check (jsonb_typeof(summary) = 'object'),
  constraint backup_imports_user_key_unique unique (user_id, backup_key)
);

create table public.backup_record_maps (
  user_id uuid not null references public.profiles(id) on delete cascade,
  ...
  imported_at timestamptz not null default timezone('utc', now()),
  primary key (user_id, entity_type, source_ref),
  constraint backup_record_maps_entity_check check (entity_type in ('wallet', 'category', 'category_rule', 'transaction', 'transfer', 'budget', 'recurring_rule')),
  constraint backup_record_maps_ref_not_empty check (char_length(trim(source_ref)) between 1 and 160),
  constraint backup_record_maps_fingerprint_check check (char_length(trim(source_fingerprint)) = 64)
);

create index backup_record_maps_user_entity_idx
  on public.backup_record_maps (user_id, entity_type);

alter table public.backup_imports enable row level security;
alter table public.backup_record_maps enable row level security;

create policy backup_imports_owner_access on public.backup_imports
for all to authenticated
using (user_id = (select auth.uid()))
with check (user_id = (select auth.uid()));

create policy backup_record_maps_owner_access on public.backup_record_maps
for all to authenticated
using (user_id = (select auth.uid()))
with check (user_id = (select auth.uid()));

create or replace function public.finexy_export_backup()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return jsonb_build_object(
    'format', 'finexy-backup',
    'version', 1,
    'exported_at', v_exported_at,
    'schema', jsonb_build_object('name', 'finexy', 'version', 'phase-28'),
    'profile', coalesce((
    ...
    'settings', coalesce((
      select jsonb_build_object(
        ...
        'default_transaction_type', s.default_transaction_type,
        'entry_mode', s.entry_mode,
        'auto_categorize', s.auto_categorize,
        'merchant_suggestions', s.merchant_suggestions,
        'confirm_before_delete', s.confirm_before_delete
      )
    ...
    'notification_preferences', coalesce((
      select jsonb_agg(jsonb_build_object(
        'preference_key', n.preference_key,
        'channel', n.channel,
        'enabled', n.enabled
      ) order by n.preference_key, n.channel)
      from public.notification_preferences n
      where n.user_id = v_user
    ), '[]'::jsonb),
    'wallets', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'wallet_' || substr(encode(extensions.digest(w.id::text, 'sha256'), 'hex'), 1, 24),
        'name', w.name,
        'currency', w.currency,
        'opening_balance', w.opening_balance::text,
        'opening_balance_at', w.opening_balance_at,
        'kind', w.kind,
        'status', w.status,
        'monthly_limit', w.monthly_limit::text,
        'institution', w.institution,
        'account_mask', w.account_mask,
        'icon_identifier', w.icon_identifier,
        'accent_identifier', w.accent_identifier
    ...
    'categories', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'category_' || substr(encode(extensions.digest(c.id::text, 'sha256'), 'hex'), 1, 24),
        'name', c.name,
        'type', c.type,
        'icon_identifier', c.icon_identifier,
        'accent_identifier', c.accent_identifier,
        'keywords', c.keywords,
        'status', c.status,
    ...
    'category_rules', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'category_rule_' || substr(encode(extensions.digest(r.id::text, 'sha256'), 'hex'), 1, 24),
        'category_ref', 'category_' || substr(encode(extensions.digest(r.category_id::text, 'sha256'), 'hex'), 1, 24),
        'field', r.field,
        'operator', r.operator,
        'value', r.value,
        'priority', r.priority,
    ...
    'transactions', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'transaction_' || substr(encode(extensions.digest(t.id::text, 'sha256'), 'hex'), 1, 24),
        'wallet_ref', 'wallet_' || substr(encode(extensions.digest(t.wallet_id::text, 'sha256'), 'hex'), 1, 24),
        'category_ref', case when t.category_id is null then null else 'category_' || substr(encode(extensions.digest(t.category_id::text, 'sha256'), 'hex'), 1, 24) end,
        'transfer_ref', case when t.transfer_id is null then null else 'transfer_' || substr(encode(extensions.digest(t.transfer_id::text, 'sha256'), 'hex'), 1, 24) end,
        'type', t.type,
        'transfer_leg', t.transfer_leg,
    ...
    'wallet_transfers', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'transfer_' || substr(encode(extensions.digest(wt.id::text, 'sha256'), 'hex'), 1, 24),
        'source_wallet_ref', 'wallet_' || substr(encode(extensions.digest(wt.source_wallet_id::text, 'sha256'), 'hex'), 1, 24),
        'destination_wallet_ref', 'wallet_' || substr(encode(extensions.digest(wt.destination_wallet_id::text, 'sha256'), 'hex'), 1, 24),
        'source_amount', wt.source_amount::text,
        'destination_amount', wt.destination_amount::text,
        'exchange_rate', wt.exchange_rate::text,
        'fee_amount', wt.fee_amount::text,
        'status', wt.status,
        'source', wt.source,
        'reference', wt.reference,
        'source_transaction_ref', case when wt.source_transaction_id is null then null else 'transaction_' || substr(encode(extensions.digest(wt.source_transaction_id::text, 'sha256'), 'hex'), 1, 24) end,
        'destination_transaction_ref', case when wt.destination_transaction_id is null then null else 'transaction_' || substr(encode(extensions.digest(wt.destination_transaction_id::text, 'sha256'), 'hex'), 1, 24) end
    ...
    'budgets', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'budget_' || substr(encode(extensions.digest(b.id::text, 'sha256'), 'hex'), 1, 24),
        'category_ref', 'category_' || substr(encode(extensions.digest(b.category_id::text, 'sha256'), 'hex'), 1, 24),
        'period_type', b.period_type,
        'period_start', b.period_start,
        'limit_amount', b.limit_amount::text,
        'currency', b.currency,
        'notes', b.notes,
        'archived_at', b.archived_at
      ) order by b.period_start, b.created_at, b.id)
      from public.budgets b
      where b.user_id = v_user
    ), '[]'::jsonb),
    'recurring_rules', coalesce((
      select jsonb_agg(jsonb_build_object(
        'ref', 'recurring_rule_' || substr(encode(extensions.digest(r.id::text, 'sha256'), 'hex'), 1, 24),
        'type', r.type,
        'wallet_ref', 'wallet_' || substr(encode(extensions.digest(r.wallet_id::text, 'sha256'), 'hex'), 1, 24),
        'category_ref', 'category_' || substr(encode(extensions.digest(r.category_id::text, 'sha256'), 'hex'), 1, 24),
      ...
      ) order by r.next_due_at, r.created_at, r.id)
      from public.recurring_transaction_rules r
      where r.user_id = v_user
    ), '[]'::jsonb)
  );
end;
...
declare
  ...
  v_transfer_leg public.transfer_leg;
  v_amount numeric;
...
begin
  ...
  create temporary table finexy_wallet_map (source_ref text primary key, target_id uuid not null) on commit drop;
  create temporary table finexy_category_map (source_ref text primary key, target_id uuid not null) on commit drop;
  create temporary table finexy_transfer_map (source_ref text primary key, target_id uuid not null) on commit drop;
  create temporary table finexy_transaction_map (source_ref text primary key, target_id uuid not null) on commit drop;
  create temporary table finexy_recurring_map (source_ref text primary key, target_id uuid not null) on commit drop;
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'wallets') as elements(value) loop
    ...
    if v_target_id is not null then
      ...
      insert into finexy_wallet_map values (v_ref, v_target_id); continue;
    end if;
    ...
    insert into finexy_wallet_map values (v_ref, v_target_id);
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'categories') as elements(value) loop
    ...
    if v_target_id is not null then
      ...
      insert into finexy_category_map values (v_ref, v_target_id); continue;
    end if;
    ...
    insert into finexy_category_map values (v_ref, v_target_id);
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'category_rules') as elements(value) loop
    v_ref := v_item->>'ref';
    if v_ref !~ '^category_rule_[A-Za-z0-9_-]{1,150}$' or not exists (select 1 from finexy_category_map where source_ref=v_item->>'category_ref') then raise exception using errcode = '22023', message = 'Category rule relationship is invalid.'; end if;
    v_fp := encode(extensions.digest(v_item::text, 'sha256'), 'hex');
    ...
    select target_id into v_category_id from finexy_category_map where source_ref=v_item->>'category_ref';
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'transactions') as elements(value) where value->>'type' <> 'transfer' loop
    v_ref := v_item->>'ref';
    if v_ref !~ '^transaction_[A-Za-z0-9_-]{1,150}$' or not exists (select 1 from finexy_wallet_map where source_ref=v_item->>'wallet_ref') or not exists (select 1 from finexy_category_map where source_ref=v_item->>'category_ref') then raise exception using errcode = '22023', message = 'Transaction relationship is invalid.'; end if;
    v_fp := encode(extensions.digest(v_item::text, 'sha256'), 'hex');
    ...
    if v_target_id is not null then
      ...
      insert into finexy_transaction_map values (v_ref, v_target_id); continue;
    end if;
    ...
    select target_id into v_wallet_id from finexy_wallet_map where source_ref=v_item->>'wallet_ref';
    select target_id into v_category_id from finexy_category_map where source_ref=v_item->>'category_ref';
    ...
    insert into finexy_transaction_map values (v_ref, v_target_id);
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'wallet_transfers') as elements(value) loop
    ...
    v_ref := v_item->>'ref';
    if v_ref !~ '^transfer_[A-Za-z0-9_-]{1,150}$' or not exists (select 1 from finexy_wallet_map where source_ref=v_item->>'source_wallet_ref') or not exists (select 1 from finexy_wallet_map where source_ref=v_item->>'destination_wallet_ref') then raise exception using errcode = '22023', message = 'Transfer relationship is invalid.'; end if;
    v_fp := encode(extensions.digest(v_item::text, 'sha256'), 'hex');
    ...
    if v_target_id is not null then
      ...
      insert into finexy_transfer_map values (v_ref, v_target_id); continue;
    end if;
    ...
    select target_id into v_wallet_id from finexy_wallet_map where source_ref=v_item->>'source_wallet_ref';
    select target_id into v_category_id from finexy_wallet_map where source_ref=v_item->>'destination_wallet_ref';
    ...
    insert into finexy_transfer_map values (v_ref, v_transfer_id);
    ...
    if v_source_transaction_ref is not null then
      ...
      insert into finexy_transaction_map values (v_source_transaction_ref, v_source_transaction_id);
    end if;
    if v_destination_transaction_ref is not null then
      ...
      insert into finexy_transaction_map values (v_destination_transaction_ref, v_destination_transaction_id);
    end if;
  ...
  if exists (
    ...
    where value->>'type'='transfer' and not exists (select 1 from finexy_transaction_map where source_ref=value->>'ref')
  ) then
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'budgets') as elements(value) loop
    v_ref := v_item->>'ref';
    if v_ref !~ '^budget_[A-Za-z0-9_-]{1,150}$' or not exists (select 1 from finexy_category_map where source_ref=v_item->>'category_ref') then raise exception using errcode = '22023', message = 'Budget relationship is invalid.'; end if;
    v_fp := encode(extensions.digest(v_item::text, 'sha256'), 'hex');
  ...
  for v_item in select value from jsonb_array_elements(p_backup->'recurring_rules') as elements(value) loop
    ...
    if v_target_id is not null then
      ...
      insert into finexy_recurring_map values (v_ref, v_target_id); continue;
    end if;
    ...
    select target_id into v_wallet_id from finexy_wallet_map where source_ref=v_item->>'wallet_ref'; select target_id into v_category_id from finexy_category_map where source_ref=v_item->>'category_ref';
    ...
    insert into finexy_recurring_map values (v_ref, v_target_id);
