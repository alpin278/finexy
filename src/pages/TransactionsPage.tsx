...
import { DeleteTransactionDialog, TransactionDetailModal, TransactionFilters, TransactionFormModal, TransactionSummary, TransactionTable, TransactionTabs, type TransactionCategoryOption, type TransactionFormValues, type TransactionTab } from '../components/transactions';
...
export function TransactionsPage() {
  ...
  const handleSaveTransaction = async (values: TransactionFormValues) => {
    ...
    try {
      ...
      const category = pageData.categories.find((item) => item.name === values.category && item.type === values.type);
  ...
  return <div className="min-w-0 w-full max-w-[calc(100vw-2rem)] space-y-6 sm:space-y-7 pb-8 animate-in fade-in-50 duration-200">
    <div className="flex min-w-0 flex-col lg:flex-row lg:items-center justify-between gap-4"><div className="min-w-0 w-full max-w-full"><h1 className="text-2xl sm:text-[32px] font-bold text-primary tracking-tight">Transactions</h1><p className="w-[calc(100vw-4rem)] max-w-full break-words whitespace-normal text-xs sm:w-auto sm:max-w-2xl sm:text-sm text-secondary mt-1">Manage, search, and audit your persisted personal income and expenses.</p></div><div className="flex items-center gap-2.5 flex-wrap"><Button variant="secondary" size="sm" leftIcon={<Download className="w-3.5 h-3.5" />} onClick={handleExport}>{exportFeedback ? 'Exported!' : 'Export CSV'}</Button><Button variant="accent" size="sm" leftIcon={<Plus className="w-3.5 h-3.5" />} onClick={openAddTransaction}>Add Transaction</Button></div></div>
