...
export function ReportsPage() {
  ...
  const handlePeriodChange = (period: ReportPeriod) => { if (period === 'custom-range') { setIsCustomRangeOpen(true); return; } setActivePeriod(period); setRange(reportRange(period, new Date(), data?.timeZone)); };
  const applyCustomRange = () => { if (customRangeError) return; setActivePeriod('custom-range'); setRange(customReportRange(customStart, customEnd, data?.timeZone)); setIsCustomRangeOpen(false); };
