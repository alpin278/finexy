  ...
  return <div className="min-w-0 space-y-6 pb-8 sm:space-y-7">
    ...
    {!loading && !error && data ? <>
      <section aria-label="Financial summary" className="grid min-w-0 items-stretch gap-4 sm:gap-5 xl:grid-cols-12">
        <div className="flex min-w-0 flex-col gap-4 sm:gap-5 md:col-span-1 xl:col-span-4"><BalanceCard amount={data.totalBalance} currency={data.reportingCurrency} period={periodLabel(data.period)} estimated={data.totalBalanceEstimated} valuationDisclosure={data.valuationDisclosure} className="h-full" /><WalletList wallets={data.wallets} onAddWallet={() => navigate('/wallets')} onWalletAction={() => navigate('/wallets')} className="h-full" /></div>
