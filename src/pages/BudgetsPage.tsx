...
const emptyData: BudgetPageData = { period: currentBudgetPeriod(), budgets: [], byCategory: {}, availablePeriods: [currentBudgetPeriod()], categories: [], summary: { activeBudgetCount: 0, totalsByCurrency: [], overBudgetCategoryCount: 0 } };
  ...
  const saveBudget = async (values: BudgetFormValues) => {
    ...
    try {
      const saved = editingBudget
        ? await updateBudget(editingBudget.id, { period: values.period, limitAmount: Number(values.monthlyLimit), currency: values.currency, notes: values.notes.trim() || null })
        : await createBudget({ categoryId: values.categoryId, period: values.period, limitAmount: Number(values.monthlyLimit), currency: values.currency, notes: values.notes.trim() || null });
      closeForm();
