...
export async function createRecurringRule(value: RecurringRuleInput) { assertOnline(); const { error } = await (supabase as any).rpc('create_recurring_transaction_rule', { p_type: value.type, p_wallet_id: value.walletId, p_category_id: value.categoryId, p_amount: value.amount, p_note: value.note, p_frequency: value.frequency, p_start_date: value.startDate, p_end_date: value.endDate || null, p_local_time: value.localTime }); if (error) throw error; }
export async function setRecurringRuleActive(id: string, active: boolean) { assertOnline(); const { error } = await (supabase as any).rpc('set_recurring_transaction_rule_active', { p_rule_id: id, p_active: active }); if (error) throw error; }
export async function archiveRecurringRule(id: string) { assertOnline(); const { error } = await (supabase as any).rpc('archive_recurring_transaction_rule', { p_rule_id: id }); if (error) throw error; }

export async function updateRecurringRule(id: string, value: RecurringRuleInput) { assertOnline(); const { error } = await (supabase as any).rpc('update_recurring_transaction_rule', { p_rule_id: id, p_type: value.type, p_wallet_id: value.walletId, p_category_id: value.categoryId, p_amount: value.amount, p_note: value.note, p_frequency: value.frequency, p_start_date: value.startDate, p_end_date: value.endDate || null, p_local_time: value.localTime }); if (error) throw error; }
