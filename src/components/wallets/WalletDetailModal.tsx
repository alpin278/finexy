...
export function WalletDetailModal({ wallet, transactions, onClose }: { wallet?: Wallet | null; transactions: Transaction[]; onClose: () => void }) {
  ...
  return <Modal isOpen onClose={onClose} title={wallet.name} description="Current balance derived from opening balance and completed transactions." maxWidth="lg" footer={<Button variant="outline" size="sm" onClick={onClose}>Close</Button>}>
    <div className="space-y-5"><div className="rounded-2xl border border-border bg-surface p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-secondary">Current balance</p><p className="mt-1 text-3xl font-bold tracking-tight text-primary">{formatWalletAmount(wallet.balance, wallet.currency)}</p><div className="mt-4 flex items-center justify-between"><span className="text-xs text-secondary">{wallet.currency} · {wallet.type ?? 'bank'}{wallet.accountMask ? ` · ${wallet.accountMask}` : ''}</span><StatusBadge status={wallet.status === 'Active' ? 'active' : 'inactive'} label={wallet.status}/></div></div>
