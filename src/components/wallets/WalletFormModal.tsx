...
export function WalletFormModal({ wallet, locale, numberFormat, onClose, onSubmit }: { wallet?: Wallet | null; locale: string; numberFormat: string; onClose: () => void; onSubmit: (values: WalletFormValues) => void }) {
  ...
  return <Modal isOpen onClose={onClose} title={wallet ? 'Edit Wallet' : 'Add Wallet'} description={wallet ? 'Update persisted wallet details. Current balance is derived from its ledger.' : 'Add a persisted account or payment source. Opening balance is the starting ledger position.'} maxWidth="lg" footer={<><Button variant="outline" size="sm" onClick={onClose}>Cancel</Button><Button variant="accent" size="sm" onClick={save}>{wallet ? 'Save Changes' : 'Add Wallet'}</Button></>}>
    <form onSubmit={(event) => { event.preventDefault(); save(); }} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label htmlFor="wallet-type" className="mb-1.5 block text-xs font-semibold text-primary">Wallet Type</label><Select id="wallet-type" value={values.type} onChange={(event) => update('type', event.target.value as WalletType)} options={walletTypes} className="h-10 w-full rounded-[12px] bg-white" aria-invalid={Boolean(errors.type)} />{errors.type && <p className="mt-1 text-xs text-danger">{errors.type}</p>}</div><div><label htmlFor="wallet-currency" className="mb-1.5 block text-xs font-semibold text-primary">Currency</label><Select id="wallet-currency" value={values.currency} onChange={(event) => update('currency', event.target.value as WalletCurrencyCode)} options={walletCurrencyOptions.map(({ value, label }) => ({ value, label }))} className="h-10 w-full rounded-[12px] bg-white" aria-invalid={Boolean(errors.currency)} />{errors.currency && <p className="mt-1 text-xs text-danger">{errors.currency}</p>}</div></div>
      ...
      <div><label htmlFor="wallet-status" className="mb-1.5 block text-xs font-semibold text-primary">Status</label><Select id="wallet-status" value={values.status} onChange={(event) => update('status', event.target.value as WalletStatus)} options={[{ value: 'Active', label: 'Active' }, { value: 'Inactive', label: 'Inactive' }]} className="h-10 w-full rounded-[12px] bg-white" /></div>
    </form>
  </Modal>;
}
