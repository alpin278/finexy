  ...
  return (
    <section aria-label={`Transaction summary in ${reportingCurrency}`} className="space-y-3">
      ...
      <div className="grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <TransactionSummaryCard label="Total Income" value={formatTotal(summary.income[reportingCurrency] ?? 0, reportingCurrency)} detail="Completed income in reporting currency" icon={<Icon name="arrow-down-left" />} iconTone="success" />
        <TransactionSummaryCard label="Total Expenses" value={formatTotal(summary.expenses[reportingCurrency] ?? 0, reportingCurrency)} detail="Completed expenses in reporting currency" icon={<Icon name="arrow-up-right" />} iconTone="danger" />
        <TransactionSummaryCard label="Net Cash Flow" value={formatTotal(summary.net[reportingCurrency] ?? 0, reportingCurrency)} detail="Income minus expenses; transfers excluded" icon={<Icon name="graph-up-arrow" />} iconTone="accent" />
      </div>
    </section>
  );
}
