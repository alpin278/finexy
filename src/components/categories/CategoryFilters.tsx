  ...
  return (
    <div className="flex min-w-0 flex-col gap-3 rounded-[20px] border border-border bg-white p-3 sm:flex-row sm:flex-wrap sm:items-center">
      <div className="min-w-0 flex-1 sm:min-w-[220px]">
        <Input aria-label="Search categories and keywords" value={values.search} onChange={(event) => onChange({ search: event.target.value })} placeholder="Search categories & keywords..." leftIcon={<Search className="h-4 w-4" />} />
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <Select aria-label="Filter by category type" value={values.type} onChange={(event) => onChange({ type: event.target.value as CategoryFilterValues['type'] })} options={[{ value: 'all', label: 'All Types' }, { value: 'expense', label: 'Expense' }, { value: 'income', label: 'Income' }]} icon={<ListFilter className="h-3.5 w-3.5" />} />
