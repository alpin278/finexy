...
export function RuleFormModal({ isOpen, categories, onClose, onSubmit }: { isOpen: boolean; categories: FinanceCategory[]; onClose: () => void; onSubmit: (values: RuleFormValues) => void }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Matching Rule" description="Create a simple local match for payees or descriptions." maxWidth="md" footer={<><Button variant="outline" size="sm" onClick={onClose}>Cancel</Button><Button variant="accent" size="sm" onClick={save}>Add Rule</Button></>}>
      <form onSubmit={(event) => { event.preventDefault(); save(); }} className="space-y-4">
        <div><label htmlFor="rule-category" className="mb-1.5 block text-xs font-semibold text-primary">Category</label><Select id="rule-category" value={values.categoryId} onChange={(event) => update('categoryId', event.target.value)} options={categories.map((category) => ({ value: category.id, label: `${category.name} · ${category.type}` }))} className="h-10 w-full rounded-[12px] bg-white px-3 pr-8" /></div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label htmlFor="rule-field" className="mb-1.5 block text-xs font-semibold text-primary">Field</label><Select id="rule-field" value={values.field} onChange={(event) => update('field', event.target.value as CategoryRuleField)} options={[{ value: 'payee', label: 'Payee' }, { value: 'description', label: 'Description' }]} className="h-10 w-full rounded-[12px] bg-white px-3 pr-8" /></div><div><label htmlFor="rule-operator" className="mb-1.5 block text-xs font-semibold text-primary">Operator</label><Select id="rule-operator" value={values.operator} onChange={(event) => update('operator', event.target.value as RuleOperator)} options={[{ value: 'contains', label: 'Contains' }, { value: 'starts_with', label: 'Starts with' }, { value: 'exact_match', label: 'Exact match' }]} className="h-10 w-full rounded-[12px] bg-white px-3 pr-8" /></div></div>
        ...
        <p className="text-[11px] text-secondary">Rules stay local to this page and do not recategorize transactions.</p>
      </form>
    </Modal>
  );
}
