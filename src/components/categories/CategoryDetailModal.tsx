    ...
    <Modal isOpen onClose={onClose} title={category.name} description="A read-only view of this persisted category." maxWidth="md" footer={<Button variant="outline" size="sm" onClick={onClose}>Close</Button>}>
      <div className="space-y-5">
        ...
        <dl className="grid grid-cols-2 gap-x-5 gap-y-4 sm:grid-cols-3">
          <div><dt className="text-[10px] font-semibold uppercase tracking-wider text-secondary">{hasBudget ? 'Qualifying transactions' : 'Prototype transactions'}</dt><dd className="mt-1 text-xs font-medium text-primary">{hasBudget ? category.budgetTransactionCount : category.transactionCount}</dd></div>
        ...
        {hasBudget && budgetStatus ? <div className="border-t border-border pt-4"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-secondary">Budget</p><p className="mt-1 text-lg font-bold text-primary">{money(category.budgetLimit!, category.budgetCurrency)}</p></div><Badge variant={budgetStatusCopy[budgetStatus].variant}>{budgetStatusCopy[budgetStatus].label}</Badge></div><div className="mt-3 flex items-center justify-between gap-3 text-xs"><span className="text-secondary">Spent this period</span><span className="font-semibold text-primary">{money(spent, category.budgetCurrency)}</span></div><ProgressBar value={spent} max={category.budgetLimit} className="mt-2" aria-label={`${category.name}: ${percentage.toFixed(1)} percent used`} /><div className="mt-2 flex justify-between text-xs text-secondary"><span>{percentage >= 100 ? 'Exceeded by' : 'Remaining'}</span><span className={percentage >= 100 ? 'font-semibold text-danger' : 'font-semibold text-primary'}>{money(Math.abs(category.budgetLimit! - spent), category.budgetCurrency)}</span></div></div> : category.type === 'expense' ? <div className="border-t border-border pt-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-secondary">Budget</p><p className="mt-1 text-sm font-semibold text-secondary">No budget</p><p className="mt-1 text-xs text-secondary">No persisted budget exists for this category in the displayed period.</p></div> : null}
