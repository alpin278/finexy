...
export function CategoryDetailModal({ category, rules, onClose }: { category?: FinanceCategory | null; rules: CategoryRule[]; onClose: () => void }) {
  ...
  return (
    <Modal isOpen onClose={onClose} title={category.name} description="A read-only view of this local category." maxWidth="md" footer={<Button variant="outline" size="sm" onClick={onClose}>Close</Button>}>
      <div className="space-y-5">
        ...
        <dl className="grid grid-cols-2 gap-x-5 gap-y-4 sm:grid-cols-3">
          <div><dt className="text-[10px] font-semibold uppercase tracking-wider text-secondary">Transactions</dt><dd className="mt-1 text-xs font-medium text-primary">{category.transactionCount}</dd></div>
