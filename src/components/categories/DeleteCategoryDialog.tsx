...
export function DeleteCategoryDialog({ category, onCancel, onConfirm }: { category?: FinanceCategory | null; onCancel: () => void; onConfirm: () => void }) {
  ...
  return (
    <Modal isOpen onClose={onCancel} title="Delete category?" description="This only removes the category from the current prototype session." maxWidth="sm" footer={<><Button variant="outline" size="sm" onClick={onCancel}>Cancel</Button><Button variant="accent" size="sm" onClick={onConfirm} className="bg-danger hover:bg-danger/90">Delete Category</Button></>}>
      <div className="space-y-3"><p className="text-sm text-primary">Are you sure you want to delete <span className="font-semibold">{category.name}</span>?</p>{category.transactionCount > 0 && <div className="rounded-xl border border-warning/30 bg-warning/10 px-3 py-2.5 text-xs leading-5 text-primary"><span className="font-semibold">Prototype transactions reference this category.</span> Deleting it will not alter the Transactions mock dataset.</div>}</div>
    </Modal>
  );
}
