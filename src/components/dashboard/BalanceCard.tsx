...
export function BalanceCard({ amount, currency, period, estimated = false, valuationDisclosure: _valuationDisclosure, className }: BalanceCardProps) {
  return (
    <Card className={cn('relative flex min-w-0 flex-col justify-between overflow-hidden p-5 sm:p-6', className)}>
      <div className="flex min-w-0 items-center justify-between gap-3">
        <span className="truncate text-xs font-semibold text-secondary sm:text-sm">{estimated ? 'Estimated Total Wealth' : 'Total Balance'}</span>
      ...
      <div className="my-4 min-w-0">
        ...
        <div className="mt-2.5 flex min-w-0 flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-secondary">{estimated ? 'Converted value' : 'Active wallets only'}</span>
        </div>
      </div>
